...
from .parse_install_yaml import installyaml_parser2
...
class InstallerImpl:
    def __init__(self, debug: bool = False):
        ...
        stamp = date_stamp()
        self._log_file = os.path.join("tmp", f"tue-get-details-{stamp}")
        ...
        self._self._tue_apt_get_updated_file = os.path.join("tmp", "tue_get_apt_get_updated")
            ...
            if execution_needed:
                ...
                if os.path.isfile(install_yaml_file):
                    if is_CI() and os.path.isfile(os.path.join(self._current_target_dir, ".ci_ignore_yaml")):
                        ...
                        cmds = installyaml_parser2(self, install_yaml_file, now)["commands"]
                        if not cmds:
