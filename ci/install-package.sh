...
then
    echo -e "\e[35m\e[1m cd ~/ros/$ROSDISTRO/system/src/$PACKAGE && git reset --hard $COMMIT \e[0m"
    docker exec tue-env bash -c "cd ~/ros/$ROSDISTRO/system/src/$PACKAGE && git reset --hard $COMMIT"
else
    echo -e "\e[35m\e[1m cd ~/ros/$ROSDISTRO/system/src/$PACKAGE && git fetch origin pull/$PULL_REQUEST/head:PULLREQUEST && git checkout PULLREQUEST \e[0m"
    docker exec tue-env bash -c "cd ~/ros/$ROSDISTRO/system/src/$PACKAGE && git fetch origin pull/$PULL_REQUEST/head:PULLREQUEST && git checkout PULLREQUEST"
fi
