...
longnames = {"v": "version", "o": "origin", "a": "suite", "c": "component", "l": "label"}
...
def compare_release(x, y):
    warnings.warn(
        "compare_release(x,y) is deprecated; please use the release_index(x) as key for sort() instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def parse_apt_policy():
    ...
    C_env = os.environ.copy()
    C_env["LC_ALL"] = "C.UTF-8"
    policy = (
        subprocess.Popen(
            ["apt-cache", "policy"], env=C_env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True
        )
        .communicate()[0]
        .decode("utf-8")
    )
...
def guess_release_from_apt(
    ...
    releases = [
        ...
        for x in releases
        if (
            x[1].get("origin", "") == origin
            and x[1].get("suite", "") not in ignoresuites
            and x[1].get("component", "") == component
            and x[1].get("label", "") == label
        )
        or (
            x[1].get("origin", "") in alternate_olabels
            and x[1].get("label", "") in alternate_olabels.get(x[1].get("origin", ""))
        )
    ]
...
def guess_debian_release():
    ...
    if not distinfo.get("CODENAME"):
        ...
        if rinfo:
            ...
            if (
                release == "1.0"
                and rinfo.get("origin") == "Debian Ports"
                and rinfo.get("label") in ("ftp.ports.debian.org", "ftp.debian-ports.org")
            ):
                release = None
