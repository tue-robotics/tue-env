...
def generate_control_file(path: str) -> dict:
    parsed = packagexml_parser(path)["parser"]

    maintainer_string = ", ".join("{maintainer} <{email}>".format(maintainer=m, email=parsed["emails"][m]) for m in parsed["maintainer"])

    system_deps = []
    for dep in parsed["depend"]:
        install_yaml = os.path.join(TUE_ENV_TARGETS_DIR, dep, "install.yaml")
        if not os.path.exists(install_yaml):
            install_yaml = os.path.join(TUE_ENV_TARGETS_DIR, "ros-" + dep, "install.yaml")
...
def create_dirs(release_dir_path: str, package_xml_path: str, timestamp: str) -> None:
    ...
    package_release_dirname = cfdict["package"] + "_" + cfdict["version"] + "-" + rev + "_" + cfdict["arch"]
